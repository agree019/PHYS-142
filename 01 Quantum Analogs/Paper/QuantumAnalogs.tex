...
%		\captionsetup{justification = justified}
	...
	Once we obtain the polar amplitude plots, we may simply compare them to the real component of the spherical harmonics. \figref{SphCompare} below shows one such comparison. The remaining polar acoustic amplitude plots can be seen in \figref{polarGraphs} and their corresponding spherical harmonics in \figref{sphereHarm} in the Appendix. From this qualitative comparison, it is possible to determine the $\ell$ value for each resonance.
